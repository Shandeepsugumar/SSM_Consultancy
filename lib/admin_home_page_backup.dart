...
class _EmployeeAttendancePageState extends State<EmployeeAttendancePage> {
  ...
  Future<void> _loadEmployeesWithAttendance() async {
    ...
    try {
      print('🔄 STARTING: Loading all employees with attendance data...');
      
      // Always ensure we have employees to display - start with basic user data
      List<Map<String, dynamic>> employeeList = [];

      // STEP 1: Get all users from users collection
      print('� STEP 1: Fetching users collection...');
      QuerySnapshot usersSnapshot;
      
      try {
        usersSnapshot = await FirebaseFirestore.instance.collection('users').get();
        print('✅ Successfully fetched users: ${usersSnapshot.docs.length} found');
      } catch (e) {
        print('❌ Failed to fetch users collection: $e');
        // Even if users fetch fails, show empty list rather than crash
        setState(() {
          _employees = [];
          _filteredEmployees = [];
          _isLoading = false;
      ...
      // STEP 2: Process each user - ALWAYS add them to the list
      for (var userDoc in usersSnapshot.docs) {
        String firebaseUid = userDoc.id;
        Map<String, dynamic> userData = userDoc.data() as Map<String, dynamic>;
        
        print('👤 Processing user: $firebaseUid');

        // Extract basic user info (ALWAYS available)
        String name = userData['name']?.toString() ?? 'Unknown User';
        String email = userData['email']?.toString() ?? 'No Email';
        String eid = userData['Eid']?.toString() ?? firebaseUid;
        ...
        // STEP 3: Try to fetch attendance data (but don't fail if it doesn't exist)
        try {
          print('📅 Fetching attendance for user: $firebaseUid');
          ...
          print('� Found ${attendanceSnapshot.docs.length} attendance records for $name');
          ...
          for (var attendanceDoc in attendanceSnapshot.docs) {
            ...
            String dateId = attendanceDoc.id;

            // Determine attendance status with multiple fallbacks
            String status = _determineAttendanceStatus(attendanceData);
            
            print('📅 Date: $dateId, Status: $status');
            ...
            attendanceRecords.add({
              'date': dateId,
              'status': status,
              'checkIn': attendanceData['checkInTime']?.toString() ?? 'N/A',
              'checkOut': attendanceData['checkOutTime']?.toString() ?? 'N/A',
            });
          }

        } catch (e) {
          print('⚠️ Could not fetch attendance for $name ($firebaseUid): $e');
          // Continue processing - don't let attendance fetch failure stop us
        }

        // STEP 4: ALWAYS add the employee to the list (with or without attendance data)
        Map<String, dynamic> employeeData = {
          ...
          'totalDays': presentDays + halfDaysPresentCount + absentDays,
          'attendanceRecords': attendanceRecords,
        };

        employeeList.add(employeeData);
        print('✅ Added employee: $name (Present: $presentDays, Half: $halfDaysPresentCount, Absent: $absentDays)');
      }

      // STEP 5: Update UI with the data we collected
      print('🎉 FINAL RESULT: Loaded ${employeeList.length} employees total');
      ...
      // STEP 6: Print final summary for debugging
      print('📈 SUMMARY:');
      for (var emp in employeeList) {
        print('   ${emp['name']}: P=${emp['presentDays']}, H=${emp['halfDaysPresentCount']}, A=${emp['absentDays']}');
      }

    } catch (e) {
      print('💥 CRITICAL ERROR in _loadEmployeesWithAttendance: $e');
      print('📍 Stack trace: ${StackTrace.current}');
      
      // Even on critical error, show empty state rather than crash
      setState(() {
        _employees = [];
        _filteredEmployees = [];
        _isLoading = false;
  ...
  // Helper method to determine attendance status with multiple fallback strategies
  String _determineAttendanceStatus(Map<String, dynamic> attendanceData) {
    ...
    if (attendanceData.containsKey('status')) {
      ...
      if (status == 'checked-in' && checkOut.isEmpty) {
        ...
        String formattedTime = attendanceData['formattedTime']?.toString() ?? '';
  ...
  String _calculateStatusFromFormattedTime(String formattedTime) {
        ...
        print(
            '✅ Processing employee: $name (EID: $eid, Firebase UID: $firebaseUid)');

        // Initialize attendance variables outside try block
        List<Map<String, dynamic>> attendanceRecords = [];
        int presentDays = 0;
        int halfDaysPresentCount = 0;
        int absentDays = 0;
        ...
        try {
          // Test if we can access the attendance document for this UID
          print('🔍 Testing attendance access for UID: $firebaseUid');

          // First, try to get the main attendance document
          DocumentSnapshot mainDoc = await FirebaseFirestore.instance
              .collection('attendance')
              .doc(firebaseUid)
              .get();

          print('📄 Main attendance document exists: ${mainDoc.exists}');
          if (mainDoc.exists) {
            print('📄 Main document data: ${mainDoc.data()}');
          }

          // Get all attendance records for this Firebase UID from dates subcollection
          ...
          print(
            '📅 Found ${dateSnapshot.docs.length} date records for Firebase UID: $firebaseUid',
          );

          // Process all attendance records for this employee
          for (var dateDoc in dateSnapshot.docs) {
            Map<String, dynamic> attendanceData =
                dateDoc.data() as Map<String, dynamic>;
            String dateId = dateDoc.id; // This is the date

            print('📆 Processing date $dateId with data: $attendanceData');
            ...
            String formattedTime =
                attendanceData['formattedTime']?.toString() ?? 'Not calculated';

            // Handle different possible field names for attendance status
            String attendanceStatus = 'absent'; // Default value
            if (attendanceData.containsKey('attendance_status')) {
              attendanceStatus =
                  attendanceData['attendance_status']?.toString() ?? 'absent';
            } else if (attendanceData.containsKey('status')) {
              String statusValue = attendanceData['status']?.toString() ?? '';
              // If only checked-in status available, assume present
              if (statusValue == 'checked-in' &&
                  checkOutTime == 'Not recorded') {
                attendanceStatus =
                    'present'; // User checked in but hasn't checked out yet
              } else if (statusValue == 'checked-out') {
                // If checked out but no attendance_status, calculate based on time
                if (formattedTime != 'Not calculated' &&
                    formattedTime.isNotEmpty) {
                  attendanceStatus = _calculateStatusFromFormattedTime(
                    formattedTime,
                  );
                } else {
                  attendanceStatus =
                      'present'; // Default if time calculation fails
                }
              }
            } else {
              // If no status field but has check-in time, assume present
              if (checkInTime != 'Not recorded') {
                if (formattedTime != 'Not calculated' &&
                    formattedTime.isNotEmpty) {
                  attendanceStatus = _calculateStatusFromFormattedTime(
                    formattedTime,
                  ...
                  attendanceStatus = 'present'; // Default for checked-in users
                }
              }
            }

            String currentDate =
                attendanceData['currentDate']?.toString() ?? dateId;

            print('📊 Attendance status for $dateId: $attendanceStatus');

            // Count attendance status
            String statusLower = attendanceStatus.toLowerCase();
            if (statusLower == 'present' || statusLower.contains('present +')) {
              presentDays++;
            } else if (statusLower == 'halfday present' ||
                statusLower == 'half day present' ||
                statusLower == 'half-day present') {
              halfDaysPresentCount++;
            } else {
              absentDays++;
            ...
            // Add record to attendance records list
            attendanceRecords.add({
              'date': dateId,
          ...
          print(
            'Processed attendance for $name ($eid) - Present: $presentDays, Half Day: $halfDaysPresentCount, Absent: $absentDays, Total Records: ${attendanceRecords.length}',
          );
        } catch (e) {
          print('Error loading attendance for Firebase UID $firebaseUid: $e');
          // Continue with empty attendance data
        }

        // Always add the employee to the list, regardless of attendance data
        employeeList.add({
          'userId': firebaseUid, // Using Firebase UID as the user identifier
          'name': name,
          'email': email,
          'Eid': eid, // Keep consistent with your data structure
          'phoneNo': phoneNo, // Use phoneNo to match your data structure
          'firebaseUid': firebaseUid, // Store Firebase UID for reference
          'attendanceRecords': attendanceRecords,
          'presentDays': presentDays,
          'halfDaysPresentCount': halfDaysPresentCount, // Use consistent naming
          'absentDays': absentDays,
          'totalDays': attendanceRecords.length,
        });

        print('Added employee: $name ($eid) to the list');
      }
      ...
      print(
        'Successfully loaded ${employeeList.length} employees with Firebase UID structure',
      );
    } catch (e) {
  ...
  Widget _buildEmployeeCard(Map<String, dynamic> employee) {
    ...
    int halfDays = employee['halfDaysPresentCount'] ?? 0; // Fixed field name
                ...
                children: [
                  ...
                  if (employee['role']?.isNotEmpty == true ||
                    ...
                    Wrap(
                      ...
                      children: [
                        if (employee['role']?.isNotEmpty == true)
                          _buildInfoChip('Role', employee['role'], Icons.work),
            ...
            if (attendanceRecords.isNotEmpty) ...[
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String status = record['attendance_status'].toLowerCase();
                  Color statusColor;
                  IconData statusIcon;
                  ...
                  return _buildDateCard(record, statusColor, statusIcon);
                },
              ),
            ],
          ],
                ...
                children: [
                  Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text(
                        _formatDateString(record['currentDate']),
                        style: TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 16,
                          color: Colors.grey[800],
  ...
  String _formatTime(String timeStr) {
    if (timeStr == 'Not recorded' || timeStr.isEmpty) {
      return 'Not recorded';
    }
  ...
  @override
  Widget build(BuildContext context) {
    // Calculate total statistics across all employees
    int totalPresent = _employees.fold(
      0,
      (sum, emp) => sum + (emp['presentDays'] as int),
    );
    int totalHalfDay = _employees.fold(
      0,
      (sum, emp) => sum + (emp['halfDays'] as int),
    );
    int totalAbsent = _employees.fold(
      0,
      (sum, emp) => sum + (emp['absentDays'] as int),
    );
    int totalRecords = _employees.fold(
      0,
      (sum, emp) => sum + (emp['totalRecords'] as int),
    );

    return Scaffold(
      appBar: AppBar(
        title: Text('Employee Attendance Management'),
      ...
      body: Column(
        children: [
          // Header Section with Search and Statistics
          Container(
            padding: EdgeInsets.all(16),
            decoration: BoxDecoration(
              gradient: LinearGradient(
            ...
            child: Column(
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Search by name, EID, email, role, department, or phone...',
        ...
        children: [
          FloatingActionButton.extended(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => SimpleAttendancePage()),
              );
            },
            backgroundColor: Colors.green[700],
            icon: Icon(Icons.bug_report, color: Colors.white),
            label: Text('Test Simple', style: TextStyle(color: Colors.white)),
            tooltip: 'Test Simple Attendance',
          ),
          SizedBox(height: 16),
          FloatingActionButton.extended(
            onPressed: _loadEmployeesWithAttendance,
            ...
            child: FloatingActionButton(
              ...
              onPressed: () async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      'Location refreshed for ${widget.employeeName}',
                    ),
                    duration: Duration(seconds: 2),
                  ),
                );
  ...
  Future<void> _fetchUserData() async {
    ...
    if (doc.exists) {
      setState(() {
        ...
        print(
          "Profile Binary String: ${_profilePicBinary?.substring(0, 50)}...",
        ...
        if (_profilePicBinary != null && _profilePicBinary!.isNotEmpty) {
          try {
            _profileImageBytes = base64Decode(_profilePicBinary!);
            print(
              "Decoded image bytes: ${_profileImageBytes!.length} bytes",
            ); // ✅ Confirm image bytes exist
          } catch (e) {
            print("Error decoding base64: $e"); // 🚨 Catch decoding errors
          }
        } else {
          print(
            "No profile image found in Firestore",
          ); // 🚨 If no binary data exists
        }
        ...
        try {
          ...
          if (todayAttendance.exists) {
            ...
            if (checkInTime.isNotEmpty && checkInTime != 'Not recorded') {
              DateTime? checkInDateTime = _parseTimeToDateTime(
                checkInTime,
                currentDate,
              );
              ...
              if (checkInDateTime != null) {
                ...
                if (checkOutTime.isEmpty ||
                  ...
                  DateTime? checkOutDateTime = _parseTimeToDateTime(
                    checkOutTime,
                    currentDate,
                  );
      ...
      child: InkWell(
        onTap: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  UserDetailScreen(userId: user['userId'], userData: user),
            ),
          );
        },
        borderRadius: BorderRadius.circular(16),
...
class UserDetailScreen extends StatefulWidget {
  ...
  const UserDetailScreen({Key? key, required this.userId, this.userData})
      : super(key: key);
